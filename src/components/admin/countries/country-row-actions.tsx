...
export function CountryRowActions({ id }: { id: string }) {
  const router = useRouter();
  const [pending, setPending] = useState(false);

  async function remove() {
    if (!confirm("Archive this country?")) return;
    setPending(true);
  ...
  return (
    <div className="flex items-center justify-end gap-1">
      <Link href={`/admin/countries/${id}`} className="rounded p-1 text-muted-foreground hover:bg-muted hover:text-foreground"><Edit className="h-4 w-4" /></Link>
