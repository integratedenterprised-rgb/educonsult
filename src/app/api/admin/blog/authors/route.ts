import { authorUpsertSchema } from "@/lib/validators/blog";
import { listAdminAuthors, upsertAuthor, SLUG_TAKEN } from "@/server/cms/admin-blog.service";
...
export async function GET() {
  ...
  return ok(await listAdminAuthors());
}
...
export async function POST(req: Request) {
  ...
  const parsed = authorUpsertSchema.safeParse(body);
  if (!parsed.success) return ApiErrors.badRequest("Invalid input", parsed.error.flatten());

  try {
    const author = await upsertAuthor(parsed.data);
