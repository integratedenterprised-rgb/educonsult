...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const row = await getPrPathway(id);
  if (!row) return ApiErrors.notFound("PR pathway");
  return ok(row);
}
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const parsed = prPathwayUpdateSchema.safeParse(body);
  if (!parsed.success) return ApiErrors.badRequest("Invalid input", parsed.error.flatten());
  try {
    const row = await updatePrPathway(id, parsed.data);
    return ok({ id: row.id });
  } catch (e) {
    if (e instanceof Error && e.message === "PATHWAY_NOT_FOUND")
      return ApiErrors.notFound("PR pathway");
    if (e instanceof Error && e.message === "COUNTRY_NOT_FOUND")
      return ApiErrors.badRequest("Unknown destination country");
    console.error("updatePrPathway failed", e);
...
export async function DELETE(_req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await ctx.params;
  try {
    await deletePrPathway(id);
    return ok({ id });
  } catch (e) {
    console.error("deletePrPathway failed", e);
    return ApiErrors.serverError();
  }
}
